...
def write_file(name,series):    #寫文件 輸入檔名 serties
    with open("0"+name+".txt", "w") as f:
        #一行一個數字 
	    for i in series:
		    f.write( str(i)+"\n")
...
write_file("DG",df_prods['diesel1'])
write_file("SOC",df_ess_soc['ess1']*100)
write_file("ESS",ess_power['ess1'])
write_file("PV",pv_power)
